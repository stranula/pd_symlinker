...
def contains_episode(files):
    episode_pattern = re.compile(
        r'(.*?)(S\d{2}E\d{2,3}(?:-E\d{2})?|\b\d{1,2}x\d{2}\b|S\d{2}E\d{2}-?(?:E\d{2})|S\d{2,3} ?E\d{2}(?:\+E\d{2})?)',
        re.IGNORECASE)
    for file in files:
    ...
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            if anime_data:
                ...
                if showdir in matched_shows:
                    matched_shows[showdir]['files'].append(src_file)
                    matched_shows[showdir]['season'].append(season_number)
                    matched_shows[showdir]['dest'].append(new_name)
                    matched_shows[showdir]['media_dir'] = media_dir
                else:
                    matched_shows[showdir] = {'files': [src_file], 'season': [season_number], 'dest': [new_name],
                                              'media_dir': media_dir}
            else:
                unmatched_anime[file] = root

    if unmatched_anime:
        log_message("WARN", "The following anime files could not be matched:")
        for file, root in unmatched_anime.items():
    ...
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            if contains_episode([file_name]):
                ...
                dest_path = os.path.join(dest_dir, showdir, season_folder)
                os.makedirs(dest_path, exist_ok=True)
                symlink_filename = os.path.join(dest_path, file_name)
    ...
    for root, dirs, files in os.walk(src_dir):
        for file in files:
            ...
            if not is_movie and not is_anime:
                ...
                multiepisode_match = re.search(
                    r'(S\d{2,3} ?E\d{2,3}E\d{2}|S\d{2,3} ?E\d{2}\+E\d{2}|S\d{2,3} ?E\d{2}\-E\d{2})', episode_identifier,
                    re.IGNORECASE)
                ...
                if multiepisode_match:
                    ...
                    episode_identifier = re.sub(r'(\d{1,2})x(\d{2})', lambda m: f's{int(m.group(1)):02d}e{m.group(2)}',
                                                episode_identifier)
                elif edge_case_episode_match:
                    episode_identifier = re.sub(r'S(\d{3}) ?E(\d{2})', lambda m: f's{int(m.group(1)):d}e{m.group(2)}',
                                                episode_identifier)

                parent_folder_name = os.path.basename(root)
                folder_name = re.sub(r'\s*(S\d{2}.*|Season \d+).*|(\d{3,4}p)', '', parent_folder_name).replace('-',
                                                                                                               ' ').replace(
                    '.', ' ')

                if re.match(r'S\d{2} ?E\d{2}', file, re.IGNORECASE):
                    show_name = re.sub(r'\s*(S\d{2}.*|Season \d+).*', '', parent_folder_name).replace('-', ' ').replace(
                        '.', ' ').strip()
                else:
